...
export function BreakingScreen({ viewer }: FeedScreenProps) {
  ...
  const loadPosts = useCallback(async () => {
    try {
      setError(null);
      const data = await fetchPosts({ viewerId: viewer.id, onlyBreaking: true });
      setPosts(data);
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={!loading ? <Text style={styles.emptyText}>No breaking news yet.</Text> : null}
      />
    </View>
  );
}
